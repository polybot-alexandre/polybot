...
def whatsapp():
    try:
        ...
        if incoming_msg in ["nova conversa", "reiniciar", "trocar idioma", "reset", "start over"]:
            historico_conversas[from_number] = []
            user_language_choice.pop(from_number, None)
            salvar_historico(historico_conversas)
            return send_text_message(from_number, "🔄 Conversa reiniciada. Escolha um novo idioma para continuar.")
        ...
        if from_number not in user_language_choice:
            texto = (
                ...
                "🔁 A qualquer momento, diga ou digite 'nova conversa', 'reiniciar' ou 'trocar idioma' para mudar."
            )
            return send_text_message(from_number, texto)
